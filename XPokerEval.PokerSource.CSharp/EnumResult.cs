...
namespace PokerEval
{
  public struct EnumResult
  {
    ...
    int nsamples;
    int nplayers;
    unsafe fixed int nwinhi[12];	/* qualifies for high and wins (no tie) */
    unsafe fixed int ntiehi[12];	/* qualifies for high and ties */
    unsafe fixed int nlosehi[12];	/* qualifies for high and loses */
    unsafe fixed int nwinlo[12];	/* qualifies for low and wins (no tie) */
    unsafe fixed int ntielo[12];	/* qualifies for low and ties */
    unsafe fixed int nloselo[12];	/* qualifies for low and loses */
    unsafe fixed int nscoop[12];	/* wins entire pot */
    ...
    unsafe fixed int nsharehi[12*13];
    unsafe fixed int nsharelo[12*13];
